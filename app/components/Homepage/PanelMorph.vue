<template>
    <section
        ref="sectionEl"
        class="relative flex items-center justify-center bg-black min-h-screen bg-center bg-contain"
        style="background-image: url('/neue-Logo.jpg'); height: 100vh;"
    >
        <svg width="100%" height="103%" class="absolute inset-0">
            <defs>
...
<script lang="ts" setup>
    ...
    onMounted(() => {
        ...
        // target the first path inside the logo group so the visible logo morphs into the hippo
        const firstPath = section.querySelector('#wb-logo') as SVGPathElement | null
        if (!firstPath) return

        gsap.to(firstPath, {
            morphSVG: { shape: '#hippo', type: 'rotational' },
            scrollTrigger: {
                trigger: section,
                start: 'top top',
                end: 'bottom top',
